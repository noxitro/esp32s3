...
| `boot` | ROMダウンロードモードで再起動(ファーム書き込み用。通常モードでも使用可) |

## ファームウェアの書き込み

デバイスは `Serial.enableReboot(false)` により、シリアルポートの開閉でリセットされません(これをしないとポート開閉のたびにUSBから切断されます)。そのため書き込み時は明示的にダウンロードモードへ入れます。方法は3つ:

1. **シリアルで `boot` を送る**(推奨) → 数秒で `VID_303A` のポートに切り替わる
2. **3キーすべてを押しながらUSBを挿す** → LEDが点灯してダウンロードモードに入る(シリアル不要)
3. XIAOの **B(BOOT)ボタンを押しながらUSBを挿す**(最後の手段)

その後:

```bash
esptool --port <新しいCOM> --chip esp32s3 --before no-reset --after hard-reset write-flash 0x10000 build/sketch.ino.bin
```

`0x10000` にアプリのみ書き込めば、保存済みの設定(NVS)は保持されます。`merged.bin` を `0x0` に書くと設定は消えます。
